...
### Cloud and Surface appearance

The Orbitals tab offers a **Surface / Cloud** appearance control aligned with the wavefunction-sign legend. Surface is the default appearance. Both modes sample the same RG16F optical field and share visibility, orientation, size mode, cutaway and camera state. Switching appearance never regenerates the worker field, and the selected appearance is retained when moving between elements.

- **Cloud** is the Beer–Lambert volume integration used throughout the original verification.
- **Surface** finds front and nested crossings of a fixed illustrative density threshold (`SURFACE_DENSITY_THRESHOLD` in `OrbitalVolume.js`, scaled by the global exposure). Each crossing is shaded with a lightweight normal estimate and composited with a user-controlled semi-transparent opacity so inner lobes remain readable. The threshold is a display envelope, not an atom boundary, calibrated probability percentile, coherent many-electron wavefunction, or measured atomic density. Sign colours remain the same red/cyan convention as Cloud mode.

d and f subshells continue to show only the d(z²) and f(z³) representatives; Surface mode does not expand them into a full angular basis set.
...
| RG16F sampling, cloud/surface ray integration and cached render target  | `components/explorer/orbitals/OrbitalVolume.js`                                                                              |
...
The orbital viewport uses the existing persistent Canvas and its own tracked view and camera. Only changed field/camera/size/cutaway/appearance/opacity/quality inputs trigger a fresh volume pass; stationary frames reuse its render target. Appearance and surface-opacity changes share the cached field and must not show “Updating shapes…”. GPU resources are replaced and disposed when fields change. Touch activation limits gesture capture to the explicit interaction period. Optional worker/WebGL failures leave configuration text and the Shell model available.
