...
const CanvasBackground = () => {
    ...
    useEffect(() => {
        if (currentElement) {
            const visibleSquares = drawSquares();
            const currentOffCanvasSquares = offCanvasSquares();
            const allSquares = mergeSquares(visibleSquares, currentOffCanvasSquares);
            // Now you have all squares, you can do something with them here
        }
        // Merge visible and off-canvas squares

    }); // Only run this effect when currentElement changes

    function mergeSquares(visibleSquares, offCanvasSquares) {
        return [...visibleSquares, ...offCanvasSquares];
    }
