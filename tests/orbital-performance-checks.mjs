...
async function orbitalState(page) {
  return page
    .locator(SURFACE)
    .evaluate((element) => JSON.parse(element.dataset.orbitalState || "null"));
}
...
async function waitForOrbital(page) {
  ...
  await page.waitForFunction((selector) => {
    ...
    return state?.renders > 0 && !state.pending;
  }, SURFACE);
...
async function tableCamera(page) {
  return page.evaluate(() => {
    ...
    return state?.camera && state?.target
 ...
 */
export async function runOrbitalPerformanceChecks(
  browser,
  ...
  const measure = async (label, settings, execute) => {
    ...
    try {
      ...
      report.measurements.push({
        ...
        status: "measured",
        ...result,
        environment: await environment(page, browser, label),
        diagnostics,
      });
    } catch (error) {
      report.measurements.push({
  ...
  try {
    for (const [name, slug] of [
      ...
      await measure(
        `${name} fresh-context mode-to-first-render`,
        DESKTOP,
        async (page) => {
          ...
          await tab.evaluate((element) =>
            window.scrollTo({
              top: scrollY + element.getBoundingClientRect().top - 140,
              behavior: "instant",
            }),
          );
          await page.evaluate((selector) => {
            window.__orbitalProfileFirstRender = {
              started: null,
              result: null,
            };
            ...
            const observer = new MutationObserver(() => {
              ...
              if (!state?.renders || state.pending) return;
              ...
              requestAnimationFrame((frameTime) => {
                timing.result = {
                  ...
                  timingKind:
                    "Click capture to first data-orbital-state publication, plus the next animation-frame callback; not isolated shader compile or GPU completion time",
                };
              });
            });
            observer.observe(document.documentElement, {
              subtree: true,
              attributes: true,
              attributeFilter: ["data-orbital-state"],
            });
          ...
          await page.waitForFunction(
            () => window.__orbitalProfileFirstRender?.result,
          );
          const timing = await page.evaluate(
            () => window.__orbitalProfileFirstRender.result,
          );
          assert.ok(
            timing.firstSurfaceVisible,
            "The first submitted view must intersect the screen",
          );
          return timing;
        },
      );
    ...
    for (const scenario of [
      {
        label: "Desktop Carbon standalone pointer rotation",
        route: "carbon/",
        settings: DESKTOP,
        touch: false,
      },
      {
        label: "Desktop Carbon table-sidebar pointer rotation",
        route: "?element=carbon",
        settings: DESKTOP,
        touch: false,
      },
      {
        label: "Emulated phone Carbon table-sidebar pointer rotation",
        route: "?element=carbon",
        name: "Carbon",
        settings: PHONE,
        touch: true,
      },
      ...["normalized", "ratios"].flatMap((sizeMode) => [
        {
          label: `Desktop Oganesson all-on ${sizeMode}`,
          route: "oganesson/",
          name: "Oganesson",
          settings: DESKTOP,
          touch: false,
        ...
        {
          label: `Emulated phone Oganesson all-on ${sizeMode}`,
          route: "?element=oganesson",
          name: "Oganesson",
          settings: PHONE,
          touch: true,
          sizeMode,
      ...
      await measure(
        ...
        scenario.settings,
        async (page, context) => {
          await page.goto(url(scenario.route), {
            waitUntil: "domcontentloaded",
          });
          await page
            .getByRole("article", {
              name: `${scenario.name || "Carbon"} details`,
              exact: true,
            })
            .waitFor();
          await page
            .getByRole("tab", { name: "Orbitals", exact: true })
            .click();
          await waitForOrbital(page);
          if (scenario.sizeMode === "ratios") {
            await page
              .getByRole("checkbox", {
            ...
            await page.waitForFunction((selector) => {
              const state = JSON.parse(
                document.querySelector(selector)?.dataset.orbitalState ||
                  "null",
              );
              return state?.sizeMode === "ratios" && !state.pending;
            }, SURFACE);
          }
          if (scenario.touch) {
            await page
              .getByRole("button", {
                name: "Interact with orbital",
                exact: true,
              })
              .tap();
            await centreSurface(page);
          ...
          await page.locator(SURFACE).evaluate((element) => {
            ...
            element.addEventListener(
              "pointerdown",
              (event) => {
                window.__orbitalProfilePointer = {
          ...
          try {
            ...
            interaction = await page.locator(SURFACE).evaluate(
              (element) =>
                new Promise((resolve, reject) => {
                  ...
                  const timeout = setTimeout(() => {
                    ...
                    reject(
                      new Error(
                        "Pointer profiling exceeded its 12-second budget",
                      ),
                    );
                  }, 12000);
                  const tick = (time) => {
                    ...
                    fullyVisible &&=
                      currentRect.top >= 0 &&
                      currentRect.bottom <= innerHeight &&
                      currentRect.left >= 0 &&
                      currentRect.right <= innerWidth;
                    const state = JSON.parse(
                      element.dataset.orbitalState || "null",
                    );
                    ...
                    if (previous !== null && frames > 6) {
                      ...
                      if (state) {
                        ...
                        const profile = renderProfiles.get(key) || {
                          width: state.width,
                          height: state.height,
                          steps: state.steps,
                          observedFrames: 0,
                    ...
                    element.dispatchEvent(
                      new PointerEvent("pointermove", {
                        ...
                        clientX:
                          x +
                          Math.sin(frames * 0.09) *
                            Math.min(70, rect.width * 0.2),
                        clientY:
                          y +
                          Math.sin(frames * 0.12) *
                            Math.min(25, rect.height * 0.08),
                      }),
                    );
                    if (intervals.length < 60)
                      ...
                      resolve({
                        ...
                        rayMarchCountDuringSamples:
                          lastRenderCount - firstRenderCount,
                        surfaceFullyVisibleThroughout: fullyVisible,
                        tablePreviewIntersectsScreen: table
                          ? table.top < innerHeight &&
                            table.bottom > 0 &&
                            table.left < innerWidth &&
                            table.right > 0
                          : null,
                        pointerType: pointer.pointerType,
                        samplingKind:
                          "60 requestAnimationFrame intervals during surface-local pointermove events, after a real captured pointerdown and six warm-up intervals",
                      });
                    }
                  };
                  raf = requestAnimationFrame(tick);
          ...
          assert.ok(
            interaction.surfaceFullyVisibleThroughout,
            "Keep the orbital surface visible throughout profiling",
          );
          assert.ok(
            Math.abs(finalState.yaw - initialState.yaw) +
              Math.abs(finalState.pitch - initialState.pitch) >
              0.01,
            "Pointer profiling must actually rotate the orbital",
          );
          const tableCameraUnchanged =
            initialTableCamera && finalTableCamera
              ? [
                  ...initialTableCamera.camera,
                  ...initialTableCamera.target,
                ].every(
                  (value, index) =>
                    Math.abs(
                      value -
                        [
                          ...finalTableCamera.camera,
                          ...finalTableCamera.target,
                        ][index],
                    ) < 0.001,
                )
              : null;
          if (tableCameraUnchanged !== null)
            assert.ok(
              tableCameraUnchanged,
              "Orbital profiling must not move the table camera",
            );
          return {
            ...interaction,
            ...percentiles(interaction.intervalsMs),
            tableCameraUnchanged,
            tableCameraVerification:
              tableCameraUnchanged === null
                ? "No development camera instrumentation available on this page/export"
                : "Compared camera position and target before/after pointer rotation",
            initialState,
            finalState,
            finalResources: {
              textures: finalState.textures,
              geometries: finalState.geometries,
            },
          };
        },
      );
    ...
    report.failed = report.measurements.filter(
